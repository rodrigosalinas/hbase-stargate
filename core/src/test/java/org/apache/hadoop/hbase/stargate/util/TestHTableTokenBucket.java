...
public class TestHTableTokenBucket extends MiniClusterTestBase {
  ...
  void doTestTokenBucketConfig() throws Exception {
  ...
  void doTestTokenBucket() throws Exception {
    HTableTokenBucket tb = new HTableTokenBucket(conf, TABLE, 
      Bytes.toBytes(USER_TOKEN));
    int last = 0;
  ...
  public void testHTableTokenBucket() throws Exception {
    doTestTokenBucketConfig();
    doTestTokenBucket();
  }
}
